...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted">{{ $index->state->name ?? '-' }} / {{ $index->district->name ?? '-' }} /
                {{ $index->office->office_name ?? '-' }}</p>
        </div>
    ...
    <div class="card p-3 mb-4">
        <dl class="row">
            <dt class="col-sm-2">Volume Year</dt>
            <dd class="col-sm-2">{{ $index->volume_year }}</dd>

            <dt class="col-sm-2">Book Number</dt>
            <dd class="col-sm-2">{{ $index->book_number }}</dd>

            <dt class="col-sm-2">Volume Number</dt>
            <dd class="col-sm-2">{{ $index->volume_number }}</dd>

            <dt class="col-sm-2">Status</dt>
            <dd class="col-sm-2">{{ ucfirst($index->status) }}</dd>
            {{-- @php $latestVerification = $index->indexVerifications->sortByDesc('verified_at')->first(); @endphp --}}
            <dt class="col-sm-2">Latest Remark</dt>
            <dd class="col-sm-2">{{ $index->status_comment ?? '-' }}</dd>
            <dt class="col-sm-2">Locked</dt>
            <dd class="col-sm-2">{{ $index->locked ? 'Yes' : 'No' }}</dd>
            <dt class="col-sm-2">Created By</dt>
            <dd class="col-sm-2">{{ $index->creator?->name ?? $index->created_by ?? '-' }}</dd>
            <dt class="col-sm-2">Created At</dt>
            <dd class="col-sm-2">{{ $index->created_at }}</dd>
            <dt class="col-sm-2">Updated At</dt>
            <dd class="col-sm-2">{{ $index->updated_at }}</dd>
        </dl>
    </div>
    ...
    @if($user && $user->isChecker() && $index->status === 'pending')
        <div class="card p-3 mb-4 border-info">
            ...
            <div class="d-flex flex-wrap gap-2">
                ...
                <button type="button" class="btn btn-sm btn-danger mx-2" data-toggle="modal" data-target="#rejectIndexModal"
                ...
                <form method="POST" action="{{ route('indexes.status.update', $index) }}" style="display:inline;">
                    ...
                    <input type="hidden" name="status_comment" value="approved..">
                    <button type="submit" class="btn btn-sm btn-success mx-2" title="Approve index">
                ...
                <div class="modal-content">
                    ...
                    <form method="POST" action="{{ route('indexes.status.update', $index) }}">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label class="form-label"><strong>Rejection Reason <span
                ...
                <tbody>
                    @foreach($index->deeds as $deed)
                        ...
                        <tr>
                            ...
                            <td>
                                <span
                                    class="badge bg-{{ $deed->status === 'approved' ? 'success' : ($deed->status === 'rejected' ? 'danger' : 'warning') }}">
                                    {{ ucfirst($deed->status) }}
                                </span>
                            </td>
                            <td>
                                ...
                                @if($user && $user->isChecker() && $deed->status === 'pending')
                                    <div class="btn-group btn-group-sm" role="group">
                                        ...
                                        {{-- <form method="POST" action="{{ route('indexes.deeds.status.update', [$index, $deed]) }}"
                                            ...
                                            @csrf
                                            <input type="hidden" name="status" value="pending">
                                            <button type="submit" class="btn btn-sm btn-warning mx-2" title="Mark as Under Process">
                                                <i class="fa fa-hourglass"></i> Process
                                            </button>
                                        </form> --}}

                                        <!-- Reject Button (with modal) -->
                                        <button type="button" class="btn btn-sm btn-danger mx-2 !important" data-toggle="modal"
                                            data-target="#rejectModal-{{ $deed->id }}" title="Reject with comment">
                                            <i class="fa fa-times"></i> Reject
                                        </button>
                                        ...
                                        <form method="POST" action="{{ route('indexes.deeds.status.update', [$index, $deed]) }}"
                                            ...
                                            @csrf
                                            <input type="hidden" name="status" value="approved">
                                            <button type="submit" class="btn btn-sm btn-success mx-2" title="Approve deed">
                                                <i class="fa fa-check"></i> Approve
                                            </button>
                                        </form>
                                    </div>
    ...
    @if($user && $user->isChecker())
        @foreach($index->deeds as $deed)
            @if($deed->status === 'pending')
                <div class="modal fade" id="rejectModal-{{ $deed->id }}" tabindex="-1"
                    aria-labelledby="rejectModalLabel-{{ $deed->id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
